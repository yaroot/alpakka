...
object S3Client {
  def create(system: ActorSystem, mat: Materializer): S3Client =
    new S3Client(S3Settings(system.settings.config.getConfig(S3Settings.DEFAULT_PATH)), system, mat)

  def create(credentials: AWSCredentials, region: String)(implicit system: ActorSystem, mat: Materializer): S3Client = {
    val settings = S3Settings(system.settings.config.getConfig(S3Settings.DEFAULT_PATH))
  ...
  def create(config: Config)(implicit system: ActorSystem, mat: Materializer): S3Client = {
    new S3Client(S3Settings(config), system, mat)
  }
}
